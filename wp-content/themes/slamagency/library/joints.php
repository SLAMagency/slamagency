...
function joints_scripts_and_styles() {
  ...
  if (!is_admin()) {
    ...
    wp_enqueue_script('jquery', 'http://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js', array(), '2.1.1', true );
    ...
    wp_enqueue_script( 'foundation-js', 'http://cdnjs.cloudflare.com/ajax/libs/foundation/5.5.2/js/foundation.min.js', array( 'jquery' ), '', true );
...
function joints_wpsearch($form) {
	...
	<div class="row collapse postfix-round">
		...
		<div class="small-10 columns">
			<input class="no-margin" type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="'.esc_attr__('Search','jointstheme').'" />
		</div>
		<div class="small-2 columns">
			<input type="submit" id="searchsubmit" class="tiny no-margin button postfix" value="'. esc_attr__('Search') .'" />
		</div>
	</div>
	</div>
	</form>';
