		...
		var updatePinState = function (forceUnpin) {
			if (_pin && _controller) {
				...
				if (!forceUnpin && _state === "DURING") { // during scene or if duration is 0 and we are past the trigger
					...
					// – – – – – – – – – – – – – – –
					// Attempted Firefox Fix
					// – – – – – – – – – – – – – – –

					//console.log(fixedPos.top, scrollDistance, fixedPos.top + scrollDistance);
					
					// var hook_offset = (.5 - _options.triggerHook) * window.innerHeight;

					if ( typeof(window.old_top) !== 'undefined' ) {
						//console.log(_pin);
						//console.log(fixedPos);
						//console.log(fixedPos.top, scrollDistance);
						//console.log(fixedPos.top - window.old_top, scrollDistance - window.old_dist);
						//
						//console.log((window.innerHeight/2), _scrollOffset.start, fixedPos.top, scrollDistance, _scrollOffset.end)
						console.log(_options.triggerHook);
						if ( -(fixedPos.top - window.old_top) !==  (scrollDistance - window.old_dist) ) {
							scrollDistance = old_dist - (fixedPos.top - window.old_top); // - hook_offset;
						}
					} else {

					}
					window.old_top = fixedPos.top;
					window.old_dist = scrollDistance;
					...
					//console.log('scrollDistance: ' + scrollDistance + ', fixedPos.top: ' + fixedPos.top);

					// set new values
					_util.css(_pin, {
						top: fixedPos.top,// - (window.innerHeight/2),
						left: fixedPos.left
					});
				} else {
					// unpinned state
